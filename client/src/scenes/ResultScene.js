...
export default class ResultScene extends Phaser.Scene {
  ...
  create(data) {
    ...
    const spr = this.add.image(W / 2, 470, spriteKey(me.id));
    const s = Math.min(360 / spr.width, 360 / spr.height);
    ...
    // 서브 pill
    pill(this, W / 2, GAME.HEIGHT - 150, won ? `${me.name} · 인술을 완성했다!` : '다음엔 더 빠르게…', {
      fill: 0x2a3a2b, textColor: won ? CSS.win : CSS.scroll, bold: true,
    });

    // 로비로
    const back = button(this, W / 2, GAME.HEIGHT - 80, 240, 62, '로비로', { fontSize: '26px' });
