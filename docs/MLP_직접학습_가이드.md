...
> **상태: 완료.** 이 문서대로 만든 MLP가 지금 게임에서 돌고 있다 (`USE_MLP = true`).
> 아래는 "무엇을 해야 하는가"이자 **"무엇이 실제로 통했는가"의 기록**이다.

## 결과 — 제3자 세션 홀드아웃 (4회)

**시험지는 `seals_2026-07-28_390f_390img`(제3자 손, 학습에 없음).** 팀원 손으로 채점하면 자기 시험 문제를 미리 본 것이라 의미가 없다.

| | 표적 재수집 **전** (3회) | 표적 재수집 **후** (4회) |
| --- | --- | --- |
| 전체 정확도 | 67.7 / 82.8 / 74.9% | **89.7 / 98.5 / 91.8%** |
| monkey | 0 / 0 / 0 | **100 × 4** |
| snake | 0 / 100 / 0 | **100 × 4** |
| rooster | 0 / 100 / 100 | **100 × 4** |
| 나머지 8종 | 100 전부 | **100 전부** |
| goat | 0 / 0 / 0 | 0 / 100 / 3 / 100 ← **여전히 불안정** |

**제3자 손에서 11종이 4회 전부 100%다.** 센트로이드의 같은 조건 성적(33.8%)과 비교할 것.

살린 건 모델이 아니라 **120장짜리 표적 재수집**이었다 — monkey·snake·rooster가 학습에 없던 **2손 자세**를 채운 것 하나로 0% → 100%가 됐다. 자세한 건 [Step 8](#step-8-성적이-안-나올-때--이-순서로).

`goat`만 4회 중 2회 실패한다(tiger·rat으로). 다만 **실패할 때 확신도가 0.51~0.89로 떨어지는 유일한 인장**이라 모델도 헷갈리고 있다는 신호다 — 다른 실패 인장들이 확신도 1.00으로 자신 있게 틀리는 것과 다르다. 그래서 `PLAYABLE_SEAL_IDS`에서 goat만 뺐다 ([constants.js](../shared/constants.js)).

> ### 추가 3회 (독립 재현, seed 20260728 / 2 / 3)
>
> ```
> 전체    85.1%   98.5%   98.2%
> goat      17%    100%    100%
> rooster    3%    100%    100%
> none      87%     80%     77%   ← 12종은 다 100%인데 여기만 떨어진다
> 나머지 10종      100% 전부
> ```
>
> 위 4회와 합쳐 **총 7회** 기준: **goat은 3회 실패, rooster는 1회 실패**(하필 기본 시드), 나머지 10종은 실패 0.
> goat을 실전에서 뺀 판단은 이 표본으로도 유지된다. rooster는 7분의 1이라 남겨도 되지만, 무너질 때는 30장 중 29장이 ox로 갈 만큼 통째로 무너지니 시연 전에 한 번 더 돌려볼 것.
>
> **그리고 진짜 약점은 인장이 아니라 `none`이다** — 3회 모두 77~87%이고, 틀릴 때 전부 `monkey`로 샌다.
> 게임에선 *가만히 있는데 원숭이가 찍히는* 형태로 나타난다. 오탐이 **확신도 1.00**으로 들어와 임계값으로는 못 잡는다
> (그래서 [config.js](../client/src/config.js)의 `ACCEPT: 0.80`을 조여봐야 소용없다 — 스윕 24조합이 전부 같은 오탐률이다).
> **`none` 샘플 다양화 수집만이 답이다.** 지금 `none`은 2세션 × 30장뿐이다.

---

> **목표**: 센트로이드를 대체할 소형 MLP를 학습시켜 `client/public/model/seal-mlp/`에 떨어뜨리고,
> **제3자 홀드아웃**으로 정직하게 채점한 뒤 게임에 꽂는다.
>
...
| 1차 학습 → 2차 평가 (2세션 시절) | 33.8% | 60.4% ± 4.1%p |
| **제3자 홀드아웃 (최종, 4세션)** | — | **89.7~98.5% · 11종 100%** |

합본 96%는 시험 문제를 미리 보고 친 시험이다. **본 적 없는 사람의 손을 맞추는 비율만이 게임에서의 성능**이다.

가운데 줄에서 맨 아래 줄로 가는 데 필요했던 건 모델이 아니라 **데이터의 조건 커버리지**였다. 그 근거가 아래 ②다.
...
위는 2세션 시절의 예측이었고, 3·4차 수집이 그걸 그대로 검증했다:

| 추가한 것 | 장수 | 결과 |
| --- | --- | --- |
| **표적 재수집** (`120f`) — goat·monkey·snake·rooster만 | **120장** | monkey·snake·rooster가 **0% → 100%** |
| 제3자 세션 (`390f`) | 390장 | 시험지로 사용 (학습 안 함) |

**120장이 앞선 780장보다 큰 일을 했다.** 늘린 게 장수가 아니라 **커버리지**였기 때문이다 — 이 네 인장은 학습 데이터에 **2손 자세가 아예 없었고**, 제3자는 2손으로 맺었다:

```
        1차     2차        3차(재수집)   제3자(시험지)
뱀      1손    1손    →    2손          2손      ← 없던 자세를 채움 → 0% → 100%
원숭이   1손    1손    →    2손          2손      ← 같음        → 0% → 100%
닭      2손   1손+2손  →   2손          2손      ← 자세 통일     → 0% → 100%
```

없는 정보는 모델이 못 만든다. 증강도 못 만든다. **그 자세를 찍어오는 것만이 답이었다.**
>
...
| 세션 | 장수 | 성격 |
| --- | --- | --- |
| `seals_2026-07-25_360f` | 360 | 1차 (팀원) · `none` 없음 |
| `seals_2026-07-27_420f` | 420 | 2차 (팀원) · `none` 30장 |
| `seals_2026-07-28_120f` | 120 | **표적 재수집** — goat·monkey·snake·rooster만 |
| `seals_2026-07-28_390f` | 390 | **제3자 세션** · `none` 30장 ← **시험지로 쓴다** |

**제3자 세션이 유일하게 정직한 시험지다.** 나머지는 전부 팀원 손이라, 그걸로 채점하면 "자기 손으로 잘 되는 것"만 확인하게 된다.
...
→ **`client/src/recognition/featuresV2.js`를 새로 만들고, MLP 경로만 쓰게 한다.** ✅ **구현 완료** — [featuresV2.js](../client/src/recognition/featuresV2.js)
...
**⚠️ 2026-07-28 3·4차 수집에도 이게 안 들어갔다** — 아직 아무 세션에도 `worldLandmarks`가 없어서 여전히 테스트 불가다:
...
✅ **구현 완료** — [tools/lib/augment.mjs](../tools/lib/augment.mjs). 아래는 그 설계 근거다.
...
✅ **구현 완료** — [tools/trainMLP.mjs](../tools/trainMLP.mjs). 실제 파일에는 아래 코드에 더해 **ACCEPT/MARGIN 스윕**과 `--seed`·`--no-save`가 붙어 있다.
...
**게이트 기준**: 제3자 홀드아웃에서 실전에 쓸 인장이 **각각** 100%에 가까워야 한다. 전체 평균이 아니라 인장별로 본다.

실제로 이 기준을 적용한 결과가 문서 맨 위 표다 — 11종은 4회 전부 100%라 실전에 넣었고, goat만 0/100/3/100으로 흔들려서 뺐다.

### 확신도를 같이 봐야 하는 이유

같은 "틀림"이라도 성질이 다르고, **대응 방법이 정반대**다:

| 실패 유형 | 확신도 | 임계값으로 걸러지나 | 해결책 |
| --- | --- | --- | --- |
| goat (4회 중 2회 실패) | **0.51~0.89** | ✅ 걸러진다 | 임계값 + 재수집 |
| 재수집 전 goat·rooster | **1.00** | ❌ 안 걸러진다 | **재수집뿐** |
| `none` 오탐 | **1.00** | ❌ 안 걸러진다 | `none` 샘플 다양화 |

**확신도 1.00으로 틀리는 건 임계값을 아무리 조여도 못 잡는다.** 게이지가 엉뚱한 인장으로 자신 있게 차오르고, ACCEPT를 올리면 정상 인장만 죽는다. 이 경우엔 임계값을 만지지 말고 데이터를 고쳐야 한다.

---

## Step 7. 게임에 붙이기

✅ **연결 완료** — [recognizer.js](../client/src/recognition/recognizer.js)의 `USE_MLP = true`, [mlpModel.js](../client/src/recognition/mlpModel.js) 구현, [config.js](../client/src/config.js)의 `MLP` 임계값까지 전부 들어가 있다.
...
| 순서 | 할 일 | 실제로 관측된 이득 |
| --- | --- | --- |
| 1 | **약한 인장 표적 재수집** (없는 자세를 채운다) | **0% → 100%** · 120장으로 3종을 되살림 ← **압도적 1위** |
| 2 | **특징 v2** (PIP 추가, Step 2) | +8.4%p (2세션 기준, 5회 확정) |
| 3 | **증강** (회전+지터+미러, Step 4) | +3.9%p · 편차 σ 4.1→2.9 |
| 4 | **실전 인장에서 빼기** | goat 제외로 불안정 요소 제거 |
| 5 | 모델 구조 | **끝까지 안 건드렸다.** 그럴 필요가 없었다 |

**모델은 한 번도 손대지 않았다.** hidden 64/32, epochs 200, dropout 0.2 그대로다. 성적을 만든 건 전부 데이터와 특징이었다.

1. **약한 인장만 표적 재수집** — 정답률 낮은 인장 3~4개만 골라 30장씩. **혼동행렬에서 상대가 누구인지보다 "학습에 없는 자세인지"를 먼저 봐라.** monkey·snake는 학습 데이터가 전부 1손인데 제3자가 2손으로 맺어서 0%였고, 그 자세를 채우자 100%가 됐다. 세션별 손 개수 분포를 찍어보는 것만으로 원인이 드러난다.
2. **사람을 늘린다** — 팀원 손만 있으면 시연에서 무너진다. 제3자 세션은 학습에 넣지 말고 **시험지로 남겨둬라** — 그게 유일하게 정직한 채점 통로다.
3. **확신도를 보고 대응을 갈라라** — 확신도가 낮게 틀리면 임계값으로 잡을 수 있고, **1.00으로 틀리면 데이터를 고치는 수밖에 없다** (Step 6 표).
4. **`worldLandmarks`로 특징 v3을 시도** — **아직 아무도 안 재봤다.** 3·4차 수집에도 저장이 안 들어가서 지금도 테스트 불가다 (Step 2 마지막). 다음 수집 때 한 줄 추가할 것.
5. **실전 인장을 줄인다** — `shared/constants.js`의 `PLAYABLE_SEAL_IDS`. 도감엔 12종, 실전엔 검증된 N종 (§4.5). 현재 goat 제외한 11종.
...
- **모델보다 데이터와 특징을 먼저 의심해라.** 관측된 이득은 표적 재수집(0%→100%) > 특징 v2(+8.4%p) > 증강(+3.9%p)이고, **모델 구조는 끝까지 안 건드렸다.** 순서를 뒤집으면 시간이 녹는다.
- **인장이 0%면 "어려운 인장"이 아니라 "학습에 없는 자세"를 의심해라.** 세션별 손 개수 분포부터 찍어봐라 — monkey·snake가 정확히 그 경우였고, 자세를 채우자 100%가 됐다.
...
- **세션마다 자세가 달라진 인장은 0%가 나온다.** 양(1차 2손 → 2차 1손)이 0%, 닭(1차 2손 → 2차 1손+2손)이 2%였다. **모델 문제가 아니라 수집 문제**이고, 더 나쁜 건 **확신도 1.00으로 자신 있게 틀린다**는 것이다 — 임계값으로 못 거른다. **수집 전에 인장별 자세(특히 손 개수)를 확정하고, 세션 간에 바꾸지 마라.** 이 하나가 이 프로젝트에서 가장 비쌌던 실수다.
...
- **증강에 하루를 쓰지 마라.** 측정된 이득은 평균 +4%p(60.4%→64.3%)다. 넣을 값어치는 있지만 **게임에 쓸 수준으로 올린 건 증강이 아니라 표적 재수집이었다**(0%→100%). 없는 자세는 증강으로 안 만들어진다.
...
- [x] `featuresV2.js` (PIP 추가, 182차원) — [featuresV2.js](../client/src/recognition/featuresV2.js)
- [x] `tools/lib/augment.mjs` — 회전+지터+미러, 드롭 제외
- [x] `tools/trainMLP.mjs` — 홀드아웃 채점 + ACCEPT/MARGIN 스윕 + 아티팩트 저장
- [x] 표적 재수집 (`120f`) → monkey·snake·rooster 0% → 100%
- [x] 제3자 세션(`390f`) 홀드아웃 4회 채점 → 11종 100%
- [x] 전 세션 학습 → `client/public/model/seal-mlp/`
- [x] `classifyMLP` 구현 + `USE_MLP = true` + `loadMLP` await
- [x] `PLAYABLE_SEAL_IDS`에서 goat 제외
- [x] 센트로이드 폴백 유지 (`USE_MLP = false` 한 줄로 복귀)
- [x] `tools/smokeMLP.mjs` — 배포 아티팩트 5초 검증

**남은 일:**

- [ ] **goat 자세 확정 후 재수집** → `trainMLP --holdout`으로 채점 → 통과하면 [constants.js](../shared/constants.js)의 주석만 해제. monkey·snake·rooster가 정확히 이 경로로 살아났다
- [ ] **rooster 안정성 재확인** — 시드를 바꿔 몇 회 더. 문서 맨 위 경고 참고
- [ ] **`none` 샘플 다양화** — 오탐이 확신도 1.00으로 들어와서 임계값으로는 못 잡는다. 여러 사람이 아무 손동작이나 찍는 게 유일한 해결책
- [ ] **collector에 `worldLandmarks` 한 줄 추가** — 특징 v3의 전제 (Step 2 마지막)
